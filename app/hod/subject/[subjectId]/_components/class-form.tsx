...
import { createClass } from "@/lib/server-actions/hod"
import { useRouter } from "next/navigation"

export function ClassForm({ subjectId }: { subjectId: string }) {
  ...
  return (
    <div className="mt-4 border rounded p-4 bg-gray-50">
      <h3 className="text-sm font-medium mb-2">Add Class</h3>
      <form action={handleSubmit} className="flex gap-2 items-end">
        <div className="flex-1">
          <label className="block text-xs font-medium text-gray-500">Class Name (e.g. 10A)</label>
          <input 
            type="text" 
            name="name" 
            required 
