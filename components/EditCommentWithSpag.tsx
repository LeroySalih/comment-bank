...
interface EditCommentWithSpagProps {
  code: string
  text: string
  /** Used for template-variable substitution before SPAG check. Defaults to 'Computer Science'. */
  subjectTitle?: string
  /** Called when the user confirms save. Return { success, error? }. */
...
export function EditCommentWithSpag({
  ...
  // Two-stage save: 'clean' → no changes; 'dirty' → must check SPAG; 'spag_ok' → ready to save
  ...
  async function handleSpagCheck() {
    ...
    const substituted = substituteVariables(text, subjectTitle || 'Computer Science')
  ...
  function resolveSpag(resolution: SpagResolution, existing: Map<number, SpagResolution>) {
    if (resolution.action === 'ignored') addIgnoredWord(resolution.match.word).catch(console.error)
    const next = new Map(existing)
    next.set(resolution.match.offset, resolution)
    setSpagResolutions(next)
    if (resolution.action !== 'ignored') setText(applyResolutions(spagOriginalText, next))
  ...
  function handleSpagRerun() { clearSpag(); handleSpagCheck() }
  ...
  function handleOpenPopup(e: React.MouseEvent<HTMLSpanElement>, match: SpagMatch) {
    ...
    setPopupPlacement(window.innerHeight - rect.bottom < 240 && rect.top > window.innerHeight - rect.bottom ? 'above' : 'below')
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="flex gap-3">
          ...
          <div className="flex-1">
            ...
            {/* Textarea OR inline SPAG annotated view */}
                          ...
                          >
                            ...
                            {editingOffset === match.offset ? (
                              <div className="pt-2 px-1 flex items-center gap-1">
                                ...
                                />
                                <button type="button" onClick={() => handleEditSubmit(match)} className="px-2 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-semibold">Apply</button>
                              </div>
                            ) : (
                              <>
                                  ...
                                  <div className="pt-2 px-1">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {match.replacements.slice(0, 6).map(r => (
                                        <button key={r} type="button" onClick={() => handleAccept(match, r)} className="px-2 py-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-700 rounded text-xs font-mono hover:bg-green-100 dark:hover:bg-green-900/50 transition-colors">
                                          {r}
                                        </button>
                                      ))}
                                    </div>
        ...
        {/* Primary action — changes with edit stage */}
